...
%#!/usr/bin/octave -qf
%GALTEC_SH   Galileo-TEC data processor (with shebang)
% GALTEC_SH processes the ground-based Galileo observables in RINEX 2.00 or
...
% SYNTAX:
%
%   galtec_sh.m SITE Y D H R MINEL
%
...
% MINEL, in degrees.
%
%   galtec_sh SITE Y D H1 H2 R MINEL
%
% specifies the period in hours [H1 H2].
%
% For example,
%
%   galtec_sh.m lpgs 2014 306 0 1 10
%
...
%
%   galtec_sh.m tash 2014 306 0 24 30 20
%
...
h1 = 450; % Maximum height of ionosphere in km
h2 = 200; % Minimum height of ionosphere in km
		...
		if ~exist(OUTFILE1,'file')
			...
			elseif exist([INFILE2(1:8) '.sp3'],'file') % gbuWWWWD.sp3
				...
				if ~exist(OUTFILE1,'file')
					...
					if exist(INFILE2,'file')
						sp3cubic(INFILE2,rate); % Create tumWWWWDiRR.mat
...
if rate==30
	...
	if exist(OUTFILE3,'file')
		...
		if exist(INFILE3,'file')
			...
			switch rv
			case 2
				system(['teqc -phc -st ' Y2 M DAY HR '0000 +dm ' DM ' -O.obs L1L5C1C5 -O.dec ' R 's ' INFILE3 ' > ' INFILE3 '.t1']); % SSSSDDD0.YYo to SSSSDDD0.YYo.t1
	...
	for i=hrng(1):hrng(2)-1
		...
		if exist(OUTFILE3,'file')
			...
			if exist(INFILE3,'file')
				...
				switch rv
				case 2
					system(['teqc -phc -O.obs L1L5C1C5 -O.dec ' R 's ' INFILE3 ' > ' INFILE3 '.t1']); % SSSSDDDA.YYo to SSSSDDA.YYo.t1
...
[lat,lon] = ionpt_e(Site,xi,yi,zi,el,ih,Domes);
