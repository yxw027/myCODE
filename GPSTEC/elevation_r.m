function el = elevation_r(Site,x,y,z,minel,Domes)
%ELEVATION_R   Calculate elevation angle for GLONASS.
% EL = ELEVATION_R('SITE',X,Y,Z,MINEL) calculates the elevation angles EL of
% the satellites at [X, Y, Z] with respect to GLONASS site SITE in 4-char above
% the minimum (cutoff) elevation angle MINEL.  All angles are in degrees and
% all lengths are in km, ECF coordinate.
%
% For example, ELEVATION_R('coco',X,Y,Z,20) returns elevation angles greater
% than 20 degrees for site 'coco'.  Any elevation angles less than MINEL are
% set to be zero.
%
% EL = ELEVATION_R('SITE',X,Y,Z,MINEL,'DOMES') further assigns the DOMES
