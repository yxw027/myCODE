function [lat,lon] = ionpt_c(Site,x,y,z,el,ih,Domes)
%IONPT_C  Estimate location of the ionospheric points for BDS.
% [LAT,LON] = IONPT_C('SITE',X,Y,Z,EL,IH) estimates the latitutde LAT and
...
% BDS site, SITE in 4-char, for the elevation angle EL in degrees and the mean
...
% Transport the elevation array if not (n x 35)
[nr,nc] = size(el);
if nr==nsat
	el = el';
	...
	nc = n;
elseif nc~=nsat
	error('EL should be in (n x 35) or (35 x n).');
end
