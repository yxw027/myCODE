function galtec(Site,yr,doy,hrng,rate,minel,ippalt,inpath,outpath)
%GALTEC   Galileo-TEC data processor
% GALTEC('SITE',Y,D,H,R,MINEL,RDCB) processes the ground-based Galileo
% observables in RINEX 2.00 or later and outputs the vertical TEC and the
% corresponding longitudes and latitudes at the ionospheric points for site
% SITE in 4-char, year Y, day-of-year D and hour H in R-second sampling above
% the minimum (cutoff) elevation angle of satellites, MINEL, in degrees.
%
% GALTEC('SITE',Y,D,[H1 H2],R,MINEL) specifies the period in hours [H1 H2].
%
% For example,
%
%   GALTEC('lpgs',2014,306,0,1,10)
%
...
% elevation angle.
%
% Notice:  Non-Galileo data will be discarded.

%   AUTHOR	:	Ho-Fang Tsai
%   SINCE	:	2012/11/27
%   VERSION	:	4.1 2016/12/15
...
tmpdir =['TMP/galileo/' Y '.' D '/'];
obsmatdir =['OBS/galileo/' Y '.' D '/'];
ORBPATH = ['ORB/'];
if ~exist(outpath)
   mkdir(outpath);
end
...
if ~exist([ORBPATH OUTFILE1],'file')
	...
	if exist([ORBPATH INFILE2],'file')
		sp3cubic([ORBPATH INFILE2],rate); % Create gbmWWWWDiRR.mat
	elseif exist([INFILE2(1:8) '.sp3'],'file') % gbmWWWWD.sp3
		sp32mat_e(w,wd); % Create gbmWWWWD.mat
		...
		if ~exist([ORBPATH OUTFILE1],'file')
			...
			if exist([ORBPATH INFILE2],'file')
				sp3cubic([ORBPATH INFILE2],rate); % Create gbuWWWWDiRR.mat
			elseif exist([INFILE2(1:8) '.sp3'],'file') % gbuWWWWD.sp3
				sp32mat_e(w,wd); % Create gbuWWWWD.mat
				sp3cubic([ORBPATH INFILE2],rate); % Create gbuWWWWDiRR.mat
			else
				OUTFILE1(1:3) = 'tum'; % tumWWWWDiRR.mat
				if ~exist([ORBPATH OUTFILE1],'file')
					INFILE2 = OUTFILE1([1:8 12:end]); % tumWWWWD.mat
					if exist([ORBPATH INFILE2],'file')
						sp3cubic([ORBPATH INFILE2],rate); % Create tumWWWWDiRR.mat
					elseif exist([INFILE2(1:8) '.sp3'],'file') % tumWWWWD.sp3
						sp32mat_e(w,wd); % Create tumWWWWD.mat
						sp3cubic([ORBPATH INFILE2],rate); % Create tumWWWWDiRR.mat
					else
...
l = S.years==yr & S.months==mon & S.days==day & S.hours>=hrng(1) & ...
	S.hours<hrng(2);
xi  = S.xi(l,1:min(nsat,size(S.xi,2))); % in km NOTICE: Galileo only
	...
	if exist([obsmatdir OUTFILE3],'file')
		...
		if exist([inpath INFILE3],'file')
			...
			case 2
				...
				system(['teqc -phc -st ' Y2 M DAY HR '0000 +dm ' DM ' -O.obs L1L5C1C5 -O.dec ' R 's ' inpath INFILE3 ' > '  pwd '/' tmpfile]); % SSSSDDD0.YYo to SSSSDDD0.YYo.t1
	...
	for i=hrng(1):hrng(2)-1
		...
		if exist([obsmatdir OUTFILE3],'file')
			...
			if exist([inpath INFILE3],'file')
				...
				case 2
                    tmpfile = [ OUTFILE3(1:9) Y2 'o.t1'];
					system(['teqc -phc -O.obs L1L5C1C5 -O.dec ' R 's ' inpath INFILE3 ' > ' pwd '/' tmpfile]); % SSSSDDDA.YYo to SSSSDDA.YYo.t1
...
el = elevation_e(Site,xi,yi,zi,minel,Domes);

% Calculate longitudes and latitudes of the ionospheric points
[lat,lon] = ionpt_e(Site,xi,yi,zi,el,ippalt,Domes);

% Deal with satellite's DCBs
galdcb = readbia('C1X-C5X',yr,doy); % DCBs in nanoseconds

% Solve ambiguity and slant TEC
[ep,stecl,amb] = ambiguity_e(Site,yr,doy,hrng,el,recdcb,galdcb,dflag,rate);
stec = stecl+amb;
...
if ~recdcb
	if dflag && nnz(stec)>=minobs
		recdcb = getrecdcb_e(Site_lc,stec,el,ep,xi,yi,zi,Domes);
	else
...
save([outpath OUTFILE],['v' SD],['s' SD],['o' SD],['a' SD],['e' SD],'ep','recdcb',...
	'galdcb','yr','doy','Domes','rlat','rlon','mintec','TimeSystem');
...
fid = fopen(LOGFILE,'w');
fprintf(fid,'Galileo site name: %s\n',[Site ' ' Domes]);
