function [lat,lon] = ionpt(Site, x, y, z, el, ih, Domes)
%IONPT  Estimate location of the ionospheric points for GPS.
% [LAT,LON] = IONPT('SITE',X,Y,Z,EL,IH) estimates the latitutde LAT and
% longitude LON of the ionospheric points in ITRF2000 ellipsoid coordinate with
% respect to the satellites at [X, Y, Z] in km, ECF coordinate, and the GPS
...
% Transport the elevation array if not (n x 32)
[nr,nc] = size(el);
if nr==nsat
	el = el';
	...
	nc = n;
elseif nc~=nsat
	error('EL should be in (n x 32) or (32 x n).');
end
