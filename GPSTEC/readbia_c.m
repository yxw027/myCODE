function dcb = readbia_c(Type,yr,doy)
%READBIA_C   Read BIA file and retrieve DCB values for BDS.
% DCB = READBIA_C('TYPE',Y,D) reads the BIA file (DLR0MGXFIN_yyyyddd0000_???
% _07D_DCB.BSX) in SINEX format with the given type, say, C2I-C7I, 4-digit year
% and 1- to 3-digit days of the year, and returns the DCB values in nanoseoncds
% for BDS.  Zeros are returned if values are not avilable.

%   AUTHOR	:	Ho-Fang Tsai
%   SINCE	:	2016/06/24
%   VERSION	:	0.7 2016/02/16
...
while(~strcmp(S(1:14),'-BIAS/SOLUTION'))
	S = fgetl(fid);
	if size(S,2)>=92+offset && ...
		(strcmp(S(2:4),'DCB') || strcmp(S(2:4),'DSB')) && ...
		S(7)=='C' && strcmp(S((31:33)+offset),Type(1:3)) && ...
		strcmp(S((36:38)+offset),Type(5:7))
