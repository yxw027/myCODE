function [lat,lon] = ionpt_e(Site,x,y,z,el,ih,Domes)
%IONPT_E  Estimate location of the ionospheric points for Galileo.
% [LAT,LON] = IONPT_E('SITE',X,Y,Z,EL,IH) estimates the latitutde LAT and
% longitude LON of the ionospheric point in ITRF2000 ellipsoid coordinate with
% respect to the satellite [X, Y, Z] in km, ECF coordinate, and the specified
% Galileo site, SITE in 4-char, for the elevation angle EL in degrees and the
% mean ionospheric height IH in km.
%
% [LAT,LON] = IONPT('SITE',X,Y,Z,EL,IH,'DOMES') further assigns the DOMES
...
nsat = 30; % Number of Galileo satellites
...
% Transport arrays if not (n x 30)
[nr,nc] = size(x);
if nr==nsat
	x = x'; y = y'; z = z';
	...
	nc = n;
elseif nc~=nsat
	error('XYZ should be in (n x 30) or (30 x n).');
end
...
% Transport the elevation array if not (n x 30)
[nr,nc] = size(el);
if nr==nsat
	el = el';
	...
	nc = n;
elseif nc~=nsat
	error('EL should be in (n x 30) or (30 x n).');
end
